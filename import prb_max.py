...
PG_TABLE = os.getenv("PG_TABLE", "prb_max")

# =========================
# File filter
# - terima 2G_daily_*.csv dan 2G_daily *.csv
# =========================
PATTERN = re.compile(r"^PRB max[\s_].*\.csv$", re.IGNORECASE)
...
def main():
    ...
    print(f"✅ Ketemu {len(files)} file PRB max*.csv (ignored non-match: {ignored})")
