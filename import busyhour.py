...
PG_TABLE = os.getenv("PG_TABLE", "busyhour_kpi")

# =========================
# File filter
# - terima 2G_daily_*.csv dan 2G_daily *.csv
# =========================
PATTERN = re.compile(r"^Busy Hour[\s_].*\.csv$", re.IGNORECASE)
...
def main():
    ...
    print("== Start ingest pl* (Graph) -> PostgreSQL ==")
    ...
    print(f"✅ Ketemu {len(files)} file Busy Hour*.csv (ignored non-match: {ignored})")
